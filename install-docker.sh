...
echo 'docker run'
docker run -d --restart=always --name blog_vue -e GIT_URL=$GIT_URL -e GIT_REF=$GIT_REF -e GIT_USERNAME=$GIT_USERNAME -e GIT_PASSWORD=$GIT_PASSWORD -e sleepTime=$sleepTime blog_vue
